...
st.set_page_config(layout="wide",page_title="Resultados 1ª Série - EM", page_icon="🎛")

with open('style.css') as f:
    st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)

st.markdown("# Resultados 1ª Série - EM")
...
def carregar_dados():
    ...
    tabela = pd.read_excel('resultados_1_serie.xlsx')
    return tabela

dados = carregar_dados()
conteudos1 = pd.read_excel('Prova_1_serie.xlsx')
# conteudos
...
colors = np.ones(len(dados_filtrados['Aluno'].unique()))
...
fig_geral = px.bar(dados_gerais, x="Aluno", y="Percent Score", 
...
colors_dis = np.ones(len(dados_filtrados['Aluno'].unique()))
...
fig_disciplina = px.bar(dados_filtrados, x="Aluno", y="Percent Score", 
...
fig_conteudo = px.bar(dados[dados["Learning Objective"]== disc_conteudo], x="Aluno", y="Percent Score", 
...
aluno = st.selectbox("Aluno", dados["Aluno"].sort_values().unique())
dados = dados[dados["Aluno"]==aluno]
values = conteudos1['Unnamed: 1'].unique()
# dados = dados[dados["Learning Objective"]==conteudos1['Unnamed: 1'].unique()]
media_aluno = dados[dados["Learning Objective"]== 'Resultados']['Percent Score'].mean()
dados = dados.loc[dados["Learning Objective"].isin(values)]

fig_aluno = px.bar(dados, x="Learning Objective", y="Percent Score", 
                               title=f'Nota do Aluno: {aluno} por Disciplina',
                                height=800, width=1280, text_auto = True, barmode='group', #color='Conteúdo',
                                ).update_xaxes(categoryorder="total descending")
fig_aluno.add_shape( # add a horizontal "target" line
            label_textposition="end", label_font_size=22, label_text=f'Média do aluno = {media_aluno} %',
            type="line", line_color="salmon", line_width=4, opacity=1, line_dash="dot",
            x0=0, x1=1, xref="paper", y0=media_aluno, y1=media_aluno, yref="y")         
fig_aluno
