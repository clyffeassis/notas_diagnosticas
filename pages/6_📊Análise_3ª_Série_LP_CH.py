...
st.set_page_config(layout="wide",page_title="Resultados 3ª Série - EM", page_icon="🎛")

with open('style.css') as f:
    st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)

st.markdown("# Resultados 3ª Série - EM")
...
def carregar_dados():
    ...
    tabela = pd.read_excel('resultados_2_serie.xls')
    return tabela

dados = carregar_dados()
# dados
conteudos1 = pd.read_excel('Prova_2_serie.xls')
# conteudos
